...
let checkout=null;
try{
checkout=sagepayCheckout({merchantSessionKey:msk,onTokenise:onToken});
...
try{
const response=await fetch("{{ url('/api/transactions') }}",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json","X-CSRF-TOKEN":document.querySelector('meta[name=csrf-token]').content},body:JSON.stringify(payload)});
let data={};
const contentType=response.headers.get("content-type")||"";
if(contentType.includes("application/json")){data=await response.json();}else{const text=await response.text();debug("Backend returned non-JSON:",text);throw new Error("Invalid response from backend.");}
...
checkout.handle3DS(data.body.three_ds_data, async function(cRes){
debug("3DS cRes received:",cRes);
const challengeResp = await fetch("{{ route('handle3DSNotification') }}",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json","X-CSRF-TOKEN":document.querySelector('meta[name=csrf-token]').content},body:JSON.stringify({transactionId:data.body.transactionId,cRes})});
const challengeData=await challengeResp.json();
debug("3DS challenge backend response:",challengeData);
if(challengeData.body?.3DSecure?.status==="Authenticated"){alert("Payment successful!");}else{showError("3DS Authentication failed.");submitBtn.disabled=false;submitBtn.textContent="Try Again";}});
...
}catch(error){console.error("Payment error:",error);showError(error.message||"An error occurred during payment");submitBtn.disabled=false;submitBtn.textContent="Try Again";}
}

submitBtn.addEventListener("click",async()=>{submitBtn.disabled=true;submitBtn.textContent="Processing…";try{await checkout.tokenise();}catch(error){console.error("Tokenization error:",error);showError("Failed to process payment. Please try again.");submitBtn.disabled=false;submitBtn.textContent="Pay Now";}});
})();
</script>
</body>
</html>
